...
# initiation
gui = Tk()
gui.geometry("450x195")
gui.title("Monthly Billing Convert Tool v2.0")
...
def createResult():
    ...
    newXls = Workbook('Monthly Billing Tool Result.xlsx')
    # 1st Pln
    ...
    newXls.close()
    if path.exists('Monthly Billing Tool Result.xlsx'):
        labelCrt = ttk.Label(
    ...
    if path.exists('Monthly Billing Tool Result.xlsx'):   
        ...
        for pdfFile in pdfFiles:
            ...
            try:
                ...
                nBook2 = load_workbook('Monthly Billing Tool Result.xlsx', data_only=True)
                nSheet2 = nBook2['Corrupted PDF']
                rowcor = (("Move this File to another if needed:", str(pdfFile)))
                nSheet2.append(rowcor)
                nBook2.save('Monthly Billing Tool Result.xlsx')
                chdir(folder)
                continue
        chdir(exeDir)
        # chdir(resultDir)
        try:
            nBook = load_workbook('Monthly Billing Tool Result.xlsx', data_only =True)
            ...
            nBook.save('Monthly Billing Tool Result.xlsx')
...
c = ttk.Button(
    gui, text="1. Create The Convert Tool Result :'Monthly Billing Tool Result.xls'", command=createResult)
...
label = ttk.Label(gui, text="Select the folder which contains pdf Files")
...
# Folder Select
btnFolder = ttk.Button(gui, text="2. Browse Folder", command=getFolderPath)
btnFolder.grid(row=5, column=2)

label1 = ttk.Label(gui, text=" ")
