...
# main part
from tkinter import Tk, StringVar, ttk, filedialog, Entry
...
# initiation
gui = Tk()
gui.geometry("660x280")
gui.title("Monthly Billing Converting Tool")
...
def createResult():
    ...
    chdir(exeDir)
    chdir(resultDir)
    # if os.path.isdir("Corrupted Files"):
    #    print("this directory exists!")
    #    pass
    # else:
    #     print("directory not found!")
    #     makedirs("Corrupted Files")
    #     print("Corrupted Folder Created!")
    newXls = Workbook('Monthly Billing Tool Extraction Data Result.xlsx')
    ...
    transaksi.write('AB1', 'Rupiah Kompensasi')
    transaksi.write('AC1', 'Rupiah TTL Terpakai')
    ...
    pln2.write('P1', 'Biaya Beban') 
    pln2.write('Q1', 'Pemakaian kWh LWBP') 
    pln2.write('R1', 'Biaya Pemk LWBP') 
    pln2.write('S1', 'Subtotal LWBP') 
    pln2.write('T1', 'Pemakaian kWh WBP')
    pln2.write('U1', 'Biaya Pemk WBP')
    pln2.write('V1', 'Subtotal WBP')
    pln2.write('W1', 'Kelebihan kVarh')
    pln2.write('X1', 'Biaya kVarh')
    pln2.write('Y1', 'Subtotal kVarh')
    pln2.write('Z1', 'Total Pemakaian')
    pln2.write('AA1', 'PTL Bruto')
    pln2.write('AB1', 'Rupiah Kompensasi')
    pln2.write('AC1', 'PPJ Total')
    pln2.write('AD1', 'RPTTL')
    pln2.write('AE1', 'Tagihan Lainnya')
    pln2.write('AF1', 'RPTL') 
    pln2.write('AG1', 'PPN') 
    pln2.write('AH1', 'Total Tagihan')
    pln2.write('AI1', 'PPJ(Persentase)')
    pln2.write('AJ1', 'PPJ')
    pln2.write('AK1', 'Jumlah Tagihan')
    pln2.write('AL1', 'Batas')
    pln2.write('AM1', 'Status')
    pln2.write('AN1', 'Tanggal')
    pln2.write('AO1', 'Biaya Keterlambatan')
    #column width
    pln2.set_column(0, 29, 15)
    ...
    newXls.close()
    if path.exists('Monthly Billing Tool Extraction Data Result.xlsx'):
        labelCrt = ttk.Label(
            gui, text="invoice result Result file created/resetted! Continue Below:")
        labelCrt.grid(row=2, column=2)
    else:
        labelCrt = ttk.Label(gui, text="Error! invoice result not created")
        labelCrt.grid(row=2, column=2)

# to get the folder name
def getFolderPath():
    folder_selected = filedialog.askdirectory()
    # folder_selected = filedialog.askopenfilename(filetypes=[("PDF files","*.pdf")])
    ...
    if path.exists('Monthly Billing Tool Extraction Data Result.xlsx'):   
        ...
        for pdfFile in pdfFiles:
            ...
            try:
                ...
                nBook2 = load_workbook('Monthly Billing Tool Extraction Data Result.xlsx', data_only=True)
                nSheet2 = nBook2['Corrupted PDF']
                rowcor = (("Move this File to another if needed:", str(pdfFile)))
                nSheet2.append(rowcor)
                nBook2.save('Monthly Billing Tool Extraction Data Result.xlsx')
                chdir(folder)
                continue
        chdir(exeDir)
        chdir(resultDir)
        try:
            nBook = load_workbook('Monthly Billing Tool Extraction Data Result.xlsx', data_only =True)
            ...
            nBook.save('Monthly Billing Tool Extraction Data Result.xlsx')
            labelCrt3 = ttk.Label(gui, text="Converted, check the result file")
            labelCrt3.grid(row=10, column=2)
        except Exception as e:
            print("Error while appending result to Excel, Error Code:", e)
            print("\n")
        labelCrt2 = ttk.Label(
            gui, text="PDF to Excel Process, check your folder if the files are converted.")
        labelCrt2.grid(row=8, column=2)
        print("Total PDF Processed : ", filesProcessed)
        
    else:
        labelCrt3 = ttk.Label(
            gui, text="Result file is not found, press the create button first.")
        labelCrt3.grid(row=10, column=2)
        ...
        if 'Pelunasan' in rapih[1][0]:
            ...
            while c < len(rapih):
                ...
                while a <len(rapih[c]):
                    ...
                    a +=1
                c+=1
            pln2data = ((idpel2, nama2, bul, gol, daya, fkm, jamnyala, stakhir, aklwbp, akwbp, stawal, awlwbp, awwbp, lwbpresetg, lwbpreset, beban, pemlwbp, blwbp, bsublwbp, pemwbp, bwbp, bsubwbp, pemkvarh, bkvarh, bsubkvarh,  totpm, ptlbruto, kompen, ptlnet, ptltagih, lain2, jumptl, ppn, totppn, ptlpersen, ptljums, jumtag, batas, status, tgl2, bk))                    
            pln2fix = []
...
# UI Design
label0 = ttk.Label(gui, text=" ")
label0.grid(row=0, column=2)

# Folder Select
btnFolder = ttk.Button(gui, text="1. Browse PDF Folder", command=getFolderPath)
btnFolder.grid(row=1, column=2)

# create the result file
label1 = ttk.Label(gui, text="Select the folder which contains pdf Files")
label1.grid(row=2, column=2)

# Output of select folder
E = Entry(gui, textvariable=folderPath, width=70)
E.grid(row=3, column=2)

# Give Empty Space
label2 = ttk.Label(gui, text=" ")
label2.grid(row=4, column=2)

c = ttk.Button(
gui, text="2. Create The Convert Tool Template Result :'Monthly Billing Tool Extraction Data Result.xls'", command=createResult)
c.grid(row=5, column=2)

# Give Empty Space
label3 = ttk.Label(gui, text=" ")
label3.grid(row=6, column=0)

# PDF -> Excel Execution
btnAppend = ttk.Button(
    gui, text="3. PDF to Excel Conversion", command=pdf_conv)
btnAppend.grid(row=7, column=2)

# Give Empty Space
label4 = ttk.Label(gui, text=" ")
label4.grid(row=8, column=0)

# Excel -> Result Execution
gui.mainloop()
